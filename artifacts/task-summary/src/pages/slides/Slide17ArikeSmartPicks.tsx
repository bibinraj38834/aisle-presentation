...
export default function Slide17ArikeSmartPicks() {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden" style={{ background: "#0C0C15", fontFamily: "var(--font-display-family)" }}>
      ...
      <div style={{ position: "absolute", bottom: "-10vh", right: "-5vw", width: "50vw", height: "50vh", background: "radial-gradient(ellipse, rgba(250,203,16,0.07) 0%, transparent 70%)", pointerEvents: "none" }} />
      ...
      <div style={{ position: "absolute", top: "5vh", left: "6vw", right: "5vw", bottom: "4vh", display: "flex", flexDirection: "column" }}>
        ...
        <p style={{ ...s(0.4, "fadeInUp"), fontFamily: "var(--font-body-family)", fontSize: "2vw", color: "#7B7F9A", lineHeight: 1.55, maxWidth: "72vw", margin: "0 0 4vh 0" }}>
          Dedicated Smart Picks API for Arike with a custom loader animation, design review sign-off, and Arike-specific fallback logic to ensure a relevant curated experience when primary picks are unavailable.
        </p>

        <div style={{ ...s(0.55, "fadeInUp"), flex: 1, display: "grid", minHeight: 0, gridTemplateColumns: "1fr 1fr", gap: "2vh 2vw" }}>
          <Img src={`${base}arike-smartpicks-1.png`} alt="Arike SmartPicks screen 1" />
          <Img src={`${base}arike-smartpicks-2.png`} alt="Arike SmartPicks screen 2" />
        </div>
      </div>

      <div style={{ position: "absolute", top: "4vh", right: "4vw" }}>
        <span style={{ fontFamily: "var(--font-body-family)", fontSize: "1.5vw", color: "rgba(250,203,16,0.4)", letterSpacing: "0.15em", fontWeight: 600 }}>ONGOING</span>
      </div>
    </div>
  );
}
