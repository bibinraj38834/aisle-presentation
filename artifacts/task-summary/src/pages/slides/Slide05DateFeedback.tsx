...
export default function Slide05DateFeedback() {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden" style={{ background: "#0C0C15", fontFamily: "var(--font-display-family)" }}>
      ...
      <div style={{ position: "absolute", top: "-10vh", right: "-5vw", width: "50vw", height: "50vh", background: "radial-gradient(ellipse, rgba(51,35,115,0.14) 0%, transparent 70%)", pointerEvents: "none" }} />
      ...
      <div style={{ position: "absolute", top: "5vh", left: "6vw", right: "5vw", bottom: "4vh", display: "flex", flexDirection: "column" }}>
        ...
        <p style={{ ...s(0.4, "fadeInUp"), fontFamily: "var(--font-body-family)", fontSize: "2vw", color: "#7B7F9A", lineHeight: 1.55, maxWidth: "72vw", margin: "0 0 3vh 0" }}>
          Private post-match feedback flow where users share whether they met and would meet again — improving recommendations and creating trust signals.
        </p>

        <div style={{ ...s(0.55, "fadeInUp"), flex: 1, display: "grid", minHeight: 0, gridTemplateColumns: "1fr 1fr", gridTemplateRows: "1fr 1fr", gap: "2vh 2vw" }}>
          <Img src={`${base}date-feedback-1.png`} alt="Date feedback prompt" />
          <Img src={`${base}date-feedback-2.png`} alt="Date feedback thanks" />
          <Img src={`${base}date-feedback-3.png`} alt="Date feedback flow 3" />
          <Img src={`${base}date-feedback-4.png`} alt="Date feedback flow 4" />
        </div>
      </div>
